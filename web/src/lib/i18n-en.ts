...
export const EN_DICT: Record<string, string> = {
  ...
  "该设备没有可用的 eSIM Profile": "This device has no eSIM profiles",
  "执行时读取当前启用的 Profile，切换到列表中的下一个，等 VoWiFi 隧道注册完成（IMS + 短信就绪）即算完成，不发短信、不打电话；隧道就绪后运营商会投递积压的短信。":
    "Each run reads the currently enabled profile, enables the next one in the list, and completes once the VoWiFi tunnel is registered (IMS + SMS ready). No SMS or call is made; the carrier delivers queued SMS once the tunnel is up.",
  "每个 Profile 的停留时间": "Dwell time per profile",
  "秒": "s",
  "从隧道就绪开始计时；切换本身约 10 秒，积压短信在注册后 1 秒内开始到达、每条约 0.65 秒。当前配置一轮约 {duration}": "Counted from tunnel readiness; a switch takes about 10 seconds, queued SMS start arriving within 1 s of registration at about 0.65 s each. One full cycle is about {duration}",
  "开始日期": "Start date",
  "开始时间": "Start time",
  "结束日期（可选）": "End date (optional)",
  "结束时间（可选）": "End time (optional)",
  "到点后任务自动停用，设备停在当时在线的号码上；留空则一直轮询": "The task disables itself at this time and the device stays on whichever profile is active; leave empty to rotate indefinitely",
  "结束日期和结束时间需同时填写，或都留空": "Set both end date and end time, or leave both empty",
  "结束时间必须晚于开始时间": "End time must be after the start time",
  "至 {time} 自动停用": "auto-disables at {time}",
  "不设结束时间": "no end time",
  "轮询中": "Rotating",
  "轮询任务运行中": "Rotation task running",
  "此设备的 eSIM Profile 会被自动任务周期性切换，手动切换或改 VoWiFi 会在下一次轮询时被覆盖。": "An automatic task keeps switching this device's eSIM profile; manual profile or VoWiFi changes are overridden on the next rotation.",
  "去自动任务页管理": "Manage in Automatic Tasks",
  ...
  发送设备: "Sending Device",
  发送号码: "Send From",
  选择发送号码: "Select a sending number",
  未就绪: "not ready",
  ...
  "仅在线": "Online only",
  "仅离线": "Offline only",
  "在位设备": "Attached devices",
  "多隧道运行中由多隧道决定启用哪个 Profile": "While multi-tunnel runs, it decides which profile is enabled",
  "多隧道正在启动或变更，读卡器暂时不可用，稍后刷新即可。": "Multi-tunnel is starting or changing and the reader is busy; refresh in a moment.",
  "多隧道运行中：可以读卡、写入、删除和改名，切换与禁用由多隧道决定。写入的新 Profile 会自动加入多隧道；删除的 Profile 会先从多隧道移除，其余线路不受影响。": "Multi-tunnel is running: you can read, download, delete and rename profiles; switching and disabling are left to multi-tunnel. A downloaded profile joins the group automatically; a deleted profile leaves the group first, and other lines keep running.",
  "全部（含已拔出）": "All (incl. unplugged)",
  "另有 {count} 台已拔出": "{count} more unplugged",
  ...
  // Multi-tunnel (multisim) display.
  "多隧道": "Multi-tunnel",
  "卡片上没有可用的 eSIM Profile": "No eSIM profile is available on this card",
  "本次多隧道包含的号码数": "Numbers included in this multi-tunnel run",
  "卡片上的 eSIM Profile 数，启动后全部建立线路": "eSIM profiles on the card; starting builds a line for every one of them",
  "可收": "receiving",
  "启动后卡片上的全部号码各自保持一条 VoWiFi 线路，不需要逐个勾选。线路按 2 条并发建立（共用一张卡的读卡器），20 个号码约需 3–5 分钟全部就绪。仅支持 EC20/EC25 AT 设备；启动前请停止本设备的自动任务。": "Starting keeps one VoWiFi line per number on the card; nothing to tick. Lines are set up two at a time (they share one card reader), so 20 numbers take roughly 3–5 minutes. EC20/EC25 AT devices only; stop this device's automatic tasks first.",
  "先读取下方 eSIM 列表，读到 Profile 后即可启动。": "Read the eSIM list below first; start once profiles are listed.",
  "启动多隧道（{n} 个号码）": "Start multi-tunnel ({n} numbers)",
  "排队等待读卡器": "Queued for the card reader",
  "{ready}/{total} 可收": "{ready}/{total} receiving",
  "第 {n} 次建立": "attempt {n}",
  "已启用但尚未接管设备": "enabled but the device is not taken over yet",
  "逐号详情、重连与续约在「eSIM → 多号码常驻接收」面板。": "Per-line details, reconnect and IMS refresh live in the eSIM → Always-on multi-number panel.",
  "多隧道运行中：每个号码保留独立的 VoWiFi 线路；此时普通 VoWiFi 运行时处于空闲，以 eSIM 标签页的多隧道面板为准。": "Multi-tunnel is running: every number keeps its own VoWiFi line; the regular VoWiFi runtime is idle, see the multi-tunnel panel on the eSIM tab.",
  "正在拆除线路": "Tearing down the line",
  "线路失败，等待重连": "Line failed, waiting to reconnect",
  "ePDG 可达": "ePDG reachable",
  "SIM 与鉴权就绪": "SIM and AKA ready",
  "已请求启动": "Start requested",
  "建立失败": "Setup failed",
  "隧道运行中断": "Tunnel dropped while running",
  "IMS 会话中断": "IMS session dropped",
  "已请求重连": "Reconnect requested",
  "已请求停止": "Stop requested",
  "IMS 已注册但短信能力未确认": "IMS registered but SMS capability unconfirmed",
  "分钟": "min",
  "小时": "h",
  "全部线路可接收短信": "All lines receiving SMS",
  "部分线路可用（{ready}/{total}）": "Some lines available ({ready}/{total})",
  "运行中，线路尚未就绪": "Running, no line ready yet",
  "配置已启用但设备尚未被接管：若长时间停留，请停止后重新启用。": "The configuration is enabled but the device has not been taken over; if this persists, stop and enable again.",
  "组状态更新于": "Group state updated",
  "前": " ago",
  "卡片当前激活": "active on card",
  "已就绪": "ready for",
  "停留": "for",
  "代理": "Proxy",
  "本线路第几次建立；每次自动重连加一": "How many times this line has been set up; each automatic reconnect adds one",
  "清理告警": "Cleanup warnings",
  "在现有隧道上做一次普通 re-REGISTER，不强制新鉴权": "Send an ordinary re-REGISTER on the existing tunnel without forcing a new AKA",
  "IMS 续约": "IMS refresh",
  "已提交 IMS 续约": "IMS refresh submitted",
  "IMS 续约失败": "IMS refresh failed",
  "收起": "Collapse",
  "会话 ID": "Session ID",
  "隧道接口": "Tunnel interface",
  "号码来源": "Number source",
  "对端认证": "Peer authentication",
  "本次建立开始": "This attempt started",
  "状态更新于": "State updated at",
  "状态序号": "State sequence",
  "激活的 Profile 只在某条线路需要读卡鉴权时才切换，其余线路靠各自已建立的隧道继续收信。": "The active profile only switches when a line needs the card for authentication; the other lines keep receiving over their own established tunnels.",
  // 通话页
  "通过线路的 IMS 会话拨打、接听和挂断电话；SIP 状态码和原因原样显示。": "Place, answer and end calls through a line's IMS session; SIP status codes and reasons are shown verbatim.",
  "拨号": "Dial",
  "线路": "Line",
  "选择线路": "Select a line",
  "拨打": "Call",
  "浏览器通话音频": "Browser call audio",
  "音频走 8 kHz 单声道 PCM，只在 VoWiFi 线路接通后可用；点拨打或接听时浏览器会请求麦克风。": "Audio is 8 kHz mono PCM and only works once a VoWiFi call is connected; the browser asks for the microphone when you dial or answer.",
  "当前线路：{line}": "Current line: {line}",
  "其他线路 {line} 有来电：{number}": "Incoming call on another line {line}: {number}",
  "接听": "Answer",
  "拒接": "Decline",
  "挂断": "Hang up",
  "当前通话": "Current call",
  "蜂窝": "Cellular",
  "没有可用线路": "No line available",
  "先在设备管理里启用 VoWiFi 或多隧道。": "Enable VoWiFi or multi-tunnel under Devices first.",
  "没有进行中的通话": "No active call",
  "拨打一个号码，或等待来电。": "Dial a number or wait for an incoming call.",
  "媒体已协商": "Media negotiated",
  "来电": "Incoming",
  "呼出": "Outgoing",
  "本方号码": "Own number",
  "接通时间": "Answered",
  "结束时间": "Ended",
  "通话 ID": "Call ID",
  "网络侧结果": "Network verdict",
  "5xx 是运营商网络拒绝，不是本地故障。": "A 5xx response is a carrier-side rejection, not a local fault.",
  "本线路通话记录": "Calls on this line",
  "这条线路自本次注册以来没有通话。": "No calls on this line since its current registration.",
  "通话中": "Active",
  "振铃": "Ringing",
  "呼出中": "Dialing",
  "保持": "Held",
  ...
  "已结束": "Ended",
  "未启用": "Off",
  "麦克风就绪，等待接通": "Microphone ready, waiting for the call to connect",
  "正在连接音频": "Connecting audio",
  "音频已接通": "Audio connected",
  "音频已断开": "Audio disconnected",
  "音频出错": "Audio error",
  "浏览器音频不可用：{reason}": "Browser audio unavailable: {reason}",
  "已发起呼叫 {number}": "Calling {number}",
};
